...
public class RawDriveImpl extends RepeatingPooledSubsystem implements Drive {
  ...
  public RawDriveImpl(CANSparkMax leftSide, CANSparkMax rightSide) {
    ...
    if (Robot.DEBUG) {
      SmartDashboard.putNumber("leftMultiplication", this.leftMultiplication);
      SmartDashboard.putNumber("rightMultiplication", this.rightMultiplication);
    }
  }
  ...
  public void drive(double leftSpeed, double rotation) {
    ...
    if (Robot.DEBUG) {
      double leftmultiplication = SmartDashboard.getNumber("leftMultiplication", 1);
      double rightmultiplication = SmartDashboard.getNumber("rightmultiplication", 1);
      if (leftmultiplication != this.leftMultiplication) {
        this.leftMultiplication = leftmultiplication;
      }
      if (rightmultiplication != this.rightMultiplication) {
        this.rightMultiplication = rightmultiplication;
      }

    }

    leftSide.set(speeds.left * this.leftMultiplication);
    rightSide.set(speeds.right * this.rightMultiplication);
  }
